    ...
    logMiddleware('permissoes', {
        buscarDadosAnteriores: async (req) => {
            ...
            const result = await db.query(
                'SELECT idusuario, modulo, cadastrar, alterar, pesquisar, acesso, master, financeiro, supremo, comercial, devs, rh, ti, camisetas FROM permissoes WHERE idusuario = $1 AND modulo = $2 AND idempresa = $3',
...
router.post('/cadastro-lote', autenticarToken({ verificarEmpresa: false }),
    logMiddleware('permissoes', {
        buscarDadosAnteriores: async (req) => {
            ...
            const result = await db.query(
                'SELECT idusuario, modulo, cadastrar, alterar, pesquisar, acesso, master, financeiro, supremo, comercial, devs, rh, ti, camisetas FROM permissoes WHERE idusuario = $1 AND idempresa = $2 AND modulo = ANY($3::text[])',
